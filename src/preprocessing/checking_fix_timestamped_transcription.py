...
def fix_transcriptions(origin_dir, destination_dir):
    ...
    for ii, filepath in enumerate(filepaths):
        ...
        for sentence_number, sentence in enumerate(sentences):
            ...
            # Search for the first word match within the text, starting from the last found index
            for i in range(search_start_index, len(df)):
                if df.loc[i, 'text'] == sentence_words[0]:
                    match = True
                    for j in range(len(sentence_words)):
                        if i + j >= len(df) or df.loc[i + j, 'text'] != sentence_words[j]:
                            match = False
                            break
                    
                    if match:
                        start_row = i
                        end_row = i + len(sentence_words) - 1
                        df.loc[start_row:end_row, 'sentence_id'] = sentence_number
                        search_start_index = end_row + 1
                        break
                else:
                    continue
        ...
        for sentence_id, group in df.groupby('sentence_id'):
            ...
            segment = {
                ...
                "compression_ratio": 1.0,  # Placeholder, adjust as needed
        ...
        with open(os.path.join('temp.json'), 'w', encoding='utf-8') as fp:
