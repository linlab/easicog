...
def embed_audio(filepath, timestamp_json, output_dir, extractor, noisereduce_audio, normalize_audio):
    ...
    features = np.nan_to_num(extractor.my_extract_static_features(audio, sr))
    
    # Save the features
    output_filename = os.path.join(output_dir, f"{recording_name}_{jj:04}.npy")
    jj+=1
    np.save(output_filename, features[None,78:]) #only get duration features
...
if __name__ == "__main__":
    ...
    for dataset_config in config["datasets"]:
        ...
        get_prosody_sentence_embeddings(
            ...
            prosody_sentence_outdir=dataset_config['prosody_rec_outdir'],
