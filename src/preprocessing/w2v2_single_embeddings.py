...
class w2v2FeatureExtractor:
    ...
    def extract_features(self, audio_chunk, downsample_factor=0):
        ...
        if downsample_factor > 1:
            features = flattened_tensor[:,::downsample_factor,:]

        return features

def embed_audio(filepath, output_dir, extractor, chunk_size=5, overlap_duration=1, downsample_factor=1):
    # Load the audio file
    sr = 16000
    ...
    for i in range(total_chunks):
        ...
        # Save the features
        output_filename = os.path.join(output_dir, f"{recording_name}_{i:02}.npy")
...
def get_w2v2_embeddings(audio_dir, w2v2_chunk_outdir, w2v2_model, w2v2_chunk_size, downsample_factor):
    # Get filepaths
...
if __name__ == "__main__":
    ...
    w2v2_chunk_size = config['w2v2_single_size']
    downsample_factor = config['downsample_factor']
    for dataset_config in config["datasets"]:
        print(dataset_config['setname'])
        get_w2v2_embeddings(
            dataset_config['audio_dir'], 
            dataset_config['w2v2_single_outdir'], 
            w2v2_model, 
            w2v2_chunk_size,
            downsample_factor)
