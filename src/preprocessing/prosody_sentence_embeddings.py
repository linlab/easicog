...
def embed_audio(filepath, timestamp_json, output_dir, extractor, noisereduce_audio, normalize_audio):
    ...
    for segment in timestamps['segments']:
        ...
        if len(chunk) < 2048:
            print(f"File {recording_name} has been padded to fill {2048} samples")
            chunk = np.pad(chunk, (0, 2048 - len(chunk) + 1), 'constant')
        ...
        np.save(output_filename, features[None,:])
...
if __name__ == "__main__":
    ...
    for dataset_config in config["datasets"]:
        ...
        get_prosody_sentence_embeddings(
            ...
            prosody_sentence_outdir=dataset_config['prosody_sentence_outdir'],
